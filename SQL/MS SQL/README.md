...
#### MERGE 절 
- Merge <br>
INSERT, UPDATE, DELETE 작업의 대상이 되는 테이블 또는 뷰를 지정한다. <br> 
- USING <br>
대상과 조인되는 데이터원본을 지정한다. <br>
- ON <br>
대상과 원본이 일치하는 부분을 확인하기 위한 조인의 조건을 지정한다. <br> 
- WHEN <br>
ON 절의 결과를 바탕으로 취할 동작을 지정한다. <br> 
...
#### 배치(BATCH) <br>
: GO 단위로 실행되는 문장의 실행 단위 <br>
: 변수의 유효 범위 <br>

IF문이나 WHILE문에 영향을 받는 문장은 그 다음 문장 뿐이다. <br>
여러 문장을 하나의 단위로 실행하고자 했을 때 BEGIN ~ END 블록으로 묶어준다. <br>
...
뷰를 사용하는 이유? <br>
1. 편의성 <br>
→ 데이터베이스의 복잡성을 가려준다(줄여준다).
2. 보안성 <br>
3. 성능향상 (인덱스된 뷰, 분할된 뷰) <br>
...
저장 프로시저의 장점 <br>
1. 빠르다. <br>
2. 매개변수를 사용할 수 있다. (융통성이 뛰어나다) <br>
3. 뷰가 가지고 있는 장점 포함 <br>

저장 프로시저는 EXEC로 실행한다는게 제한이다. <br>

함수란? 리턴값을 갖는 저장프로시저 이다. <br>
1. 스칼라 함수 (단일한 결과를 반환하는 함수) <br>
2. 인라인 테이블 값 함수 <br>
3. 다중문 테이블 값 함수 <br>

함수 문법 <br>
1. 매개변수는 괄호로 묶는다. <br>
2. RETURNS 반환될 데이터 형식을 지정한다. <br>
3. 함수 본문은 BEGIN ~ END 블록으로 묶어준다. <br>
4. 함수 본문 마지막에 RETURN문과 함께 반환될 값을 정의한다. <br>

사용자 정의 함수는 두 부분의 이름을 써주어야 한다. <br>
- 스키마명.함수명 <br>

트리거란? <br>
: 자동으로 실행되는 저장프로시저이다. <br>
: 테이블에 기반해서 동작한다. <br>

트리거를 사용하는 이유? <br>
: 업무규칙을 반영하기 위해서 <br>

실시간으로 데이터를 처리해야 하는가? <br>
1) 그렇다. 트리거를 사용하고, 서버의 용량을 증설한다. <br>
2) 아니다. 트리거를 사용하지 않고, 커서를 사용한다. <br>
...
- 압축을 통해서 얻을 수 있는 이점은? <br>
1. 공간에 대한 이점 <br>
2. 성능에 대한 이점 <br>
3. 성능에 대한 이점과 공간에 대한 이점이 있다. <br>

읽기 전용인 경우 압축한다. 입력, 수정, 삭제하는 업무용 테이블은 기본 상태 유지 <br>

인덱스가 성능을 향상시키는 방법 <br>

페이지(8kb) <br>
: 물리적으로 데이터를 저장하는 단위 <br>

데이터페이지 <br>
인덱스데이지 <br>

힙: 한 테이블의 데이터들을 저장하고 있는 데이터 페이지들의 관리단위 <br>

인덱스를 통해서 데이터를 빨리 검색할 수 있는 이유? <br>
: 인덱스는 항상 정렬된 상태를 유지한다. <br>

SQL Server의 인덱스 유형  <br>
1. 클러스터드 인덱스 : 사전 <br>
: 해당 열을 기준으로 데이터들을 물리적으로 정렬시킨다. <br>
: 데이터 페이지들을 클러스터드 인덱스의 리프레벨로 사용한다. <br>
: 한 테이블에 한 번만 만들 수 있다. <br>

2. 넌 클러스터드 인덱스 : 일반 책 뒤에 있는 인덱스 <br>
: 물리적으로 데이터를 정렬시키지 않고, 데이터 페이지와는 별도로 데이터들의 위치정보를 인덱스로 구성한다. <br>
: 한 테이블에 249개의 넌 클러스터드 인덱스를 만들 수 있다. <br>

질의의 유형 <br>
1. 포인트 질의 <br>
: 조회 결과가 하나인 경우의 질의(키를 기준으로 조회) <br>
2. 범위 질의 <br>
: 조회 결과가 여러 개인 경우의 질의 <br>
3. 커버드 질의 <br>
: 조회의 조건과 조회의 대상 테이블이 모두 인덱스로 구성된 경우의 질의 <br>

인덱스의 상위 레벨은 하위 페이지들의 첫번째 레코드 정보를 갖는다. <br>
테이블 스캔 : 최악 <br>

인덱스 <br>
1) 시크(SEEK) : 최선 <br>
2) 스캔(SCAN) : 개선해야 한다. <br>

인덱스를 통해서 검색하는 경우 자신보다 작은 가장 큰 값을 이용한다. <br>

검색 성능은? <br>
포인트 질의 : 클러스터드 인덱스가 좀 더 빠르다.  <br>
범위 질의 <br>
: 클러스터드 인덱스 무지 빠르다. <br>
: 넌 클러스터드 인덱스는 무지 느리다. 심한 경우 테이블 스캔보다 못할 수 있다. <br>
: 넌 클러스터드 인덱스가 유용한 일반적인 경우 1% 미만 조회 <br>

클러스터드 인덱스가 좀 더 빠른 이유는? <br>
: 넌 클러스터드 인덱스의 경우 인덱스 리프레벨에서 데이터 페이지로 이동해서 데이터를 찾지만, 클러스터드 인덱스의 경우 인덱스의 리프레벨에서 데이터를 찾는다. <br>

페이지 분할 <br>
새로운 페이지가 만들어지고, 약 반정도의 데이터가 새로운 페이지에 옮겨진다. 그렇게 확보된 공간에 새로운 데이터가 저장된다. <br>

데이터 입력 시 클러스터드 인덱스의 오버해드가 더 심하다. <br>
레코드가 끼어들어가야 하기 때문에 <br>

데이터 입력 시 부하를 최소화하는 방법? <br>
인덱스 페이지에 빈 공간을 확보한다. <br>

서버레벨에서 인덱스 채우기 비율을 설정한다. <br>

조회과다 - 95% <br>
기본 - 90% <br>
입력과다 - 80% <br>

인덱스 조각모음 - 30% 조각화 미만 : 조각모음 <br>
인덱스 다시 만들기 - 30% 조각화 이상 : 다시만들기 <br>

테이블에 첫 번째 인덱스를 고를 때 고려해야 하는 인덱스 유형은? <br>
: 클러스터드 인덱스 <br>

인덱스의 사이즈<br>
: 같은 역할을 할 수 있다면 보다 적은 사이즈의 열에 인덱스를 만드는 것이 좋다. <br>

- 인덱스 만들기  <br>

기본키 제약조건 : 유니크한 클러스터드 인덱스가 기본으로 만들어진다. <br>
유니크 제약조건 : 유니크한 넌클러스터드 인덱스가 기본으로 만들어진다. <br>

로그인이 있다고 하더라도 데이터베이스에 연결하기 위해서는 해당 데이터베이스에 사용자로 등록되어야 한다. <br>
SysAdmin 서버역할은 각각의 데이터베이스에 있는 DBO계정과 자동 매핑된다. <br>

사용자로 연결 시 권한을 가지고 있는 개체들만 볼 수 있다. <br>

#### 일반적인 백업의 종류
보통, 복사 : 선택된 모든 파일과 폴더를 통째로 백업 받는다. 보통 - 백업표시 O, 복사 - 백업표시 X<br>
증분, 차등 : 마지막 백업 이우헤 변경된 내용을 백업 받는다. 증분 - 백업표시 O, 차분 - 백업표시 X <br>
매일 : 당일 변경 내용을 백업 받는다.  백업표시 X<br>

데이터 저장과 관련된 3가지 프로세스 <br>
1. CHECK POINT : 현재 진행중인 모든 내용을 데이터 파일과 로그 파일에 기록한다. <br>
2. LAZY WRITER : CHECK POINT와 동일하다. (동작 시점 다름) <br>
3. LOG WRITER : 트랜잭션이 커밋되면 커밋된 트랜잭션 로그를 트랜잭션 로그파일에 기록한다. <br>

자동복구 모드 : SQL Server 서비스가 시작될 때 등록된 데이터베이스들의 데이터 파일과 로그 파일을 비교한다. <br>
...
트랜잭션의 기본 개념 : ALL OR NOTHING <br>

트랜잭션 내에서 제약조건에 대한 오류가 발생하면 해당 문장만 실패한다. <br>
