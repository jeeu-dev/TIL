...
-------
### 완강 후기
> 2019-01-21 ~ 2019-02-07
- Java, Spring, SQL에 대해 쉽게, 이해가게 설명해주셔서 좋았다. <br>
- 이제 적어도 Spring 프레임워크를 사용하며 코드를 읽고 수정할 수는 있을 것 같다. 
...
- 리다이렉트를 사용해야 하는 경우가 많은 경우 HandlerInterceptor를 이용할 수 있다. <br>
...
dataSource = new DriverManagerDataSource(); <br>
dataSource.setDriverClass(driver); <br>
dataSource.setJdbcUrl(url); <br>
dataSource.setUser(userid); <br>
dataSource.setPassword(userpw); <br>
- ComboPooledDataSource를 쓰면 예외처리를 해줘야함 try ~ catch<br> 
...
```
- 취향껏 xml을 쓸지 .java를 쓸지 고르면 된다. <br>
- 커넥션풀은 반드시 써야되는 기술이기 때문에 확실히 알고가면 좋다.
